...
public sealed class AppSettingsStore(string path, AppSettings document)
{
    ...
    public static AppSettingsStore LoadDefault()
    {
        ...
        if (!File.Exists(path))
        {
            return new AppSettingsStore(path, CreateDefaultDocument());
        }

        string json = File.ReadAllText(path);
        AppSettings? document = JsonSerializer.Deserialize<AppSettings>(json, JsonOptions);
        return new AppSettingsStore(path, document ?? CreateDefaultDocument());
    }
    ...
    public void SaveGame(string oldId, string newId, GameProfile profile)
    {
        ...
        try
        {
            AppSettings latest = LoadFromPath(Path);
    ...
    private static AppSettings CreateDefaultDocument()
    {
    ...
    private static AppSettings LoadFromPath(string path)
    {
        ...
        string json = File.ReadAllText(path);
        return JsonSerializer.Deserialize<AppSettings>(json, JsonOptions) ?? CreateDefaultDocument();
    }
