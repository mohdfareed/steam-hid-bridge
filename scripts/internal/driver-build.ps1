...
$driverOut = Join-Path $repoRoot "driver\obj\SteamHidBridge.VirtualMouse\$Platform\$Configuration"
$driverObj = Join-Path $repoRoot "driver\obj\SteamHidBridge.VirtualMouse\$Platform\$Configuration"
...
& $msbuild $driverProject /nologo /v:quiet /t:Restore /p:Configuration=$Configuration /p:Platform=$Platform
if ($LASTEXITCODE -ne 0) {
...
Invoke-QuietNative "Compiling virtual mouse driver" {
...
Invoke-QuietNative "Linking virtual mouse driver" {
...
Invoke-QuietNative "Stamping virtual mouse driver INF" {
