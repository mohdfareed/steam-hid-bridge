...
dotnet format (Join-Path $root "SteamHidBridge.slnx") --verify-no-changes --no-restore

dotnet build (Join-Path $root "app\SteamHidBridge.App\SteamHidBridge.App.csproj") --configuration $Configuration --no-restore
dotnet build (Join-Path $root "protocol\SteamHidBridge.Protocol.Tests\SteamHidBridge.Protocol.Tests.csproj") --configuration $Configuration --no-restore
